...
export class MidiKeyManager {
	...
	_handleKeyboardEvent(event, up) {
		...
		if (event.isComposing)
			return; // Ignore IME composition
		if (!event.key && !event.code)
			return; // Some browsers fire keyup and keydown events when autocompleting values.
		let context = {
			event: event,
			key: event.code,
	...
	initKeyMappings() {
		...
		Hooks.on('renderDialog', (dialog, html, data) => {
			//@ts-expect-error
			if (CONFIG.debug.keybindings)
				console.log("midi-qol | dialog rendered - releasing keys");
			//@ts-expect-error
		...
		Hooks.on('renderDialogV2', (dialog, html, data) => {
			//@ts-expect-error
			if (CONFIG.debug.keybindings)
				console.log("midi-qol | dialog v2 rendered - releasing keys");
			//@ts-expect-error
			game.keyboard?.releaseKeys({ force: true });
		});
	}
